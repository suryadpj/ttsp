...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $chart_input = DB::table('datakpi_result as a')->whereyear('periode',date('Y'))->where('deleted',0)->select(DB::raw('ROUND((SUM(IF(a.periode="2024-01-01",1,0))/23)*100) AS jan'),DB::raw('ROUND((SUM(IF(a.periode="2024-02-01",1,0))/23)*100) AS feb'),DB::raw('ROUND((SUM(IF(a.periode="2024-03-01",1,0))/23)*100) AS mar'),DB::raw('ROUND((SUM(IF(a.periode="2024-04-01",1,0))/23)*100) AS apr'),DB::raw('ROUND((SUM(IF(a.periode="2024-05-01",1,0))/23)*100) AS mei'),DB::raw('ROUND((SUM(IF(a.periode="2024-06-01",1,0))/23)*100) AS jun'),DB::raw('ROUND((SUM(IF(a.periode="2024-07-01",1,0))/23)*100) AS jul'),DB::raw('ROUND((SUM(IF(a.periode="2024-08-01",1,0))/23)*100) AS aug'),DB::raw('ROUND((SUM(IF(a.periode="2024-09-01",1,0))/23)*100) AS sep'),DB::raw('ROUND((SUM(IF(a.periode="2024-10-01",1,0))/23)*100) AS okt'),DB::raw('ROUND((SUM(IF(a.periode="2024-11-01",1,0))/23)*100) AS nov'),DB::raw('ROUND((SUM(IF(a.periode="2024-12-01",1,0))/23)*100) AS des'))->first();
